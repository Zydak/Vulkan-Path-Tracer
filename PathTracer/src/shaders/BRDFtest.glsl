...
void EvaluateBRDF(inout BRDFEvaluateData data, in Material mat, in Surface surface)
{
    ...
    float TotalWeightInv = 1.0f / (diffuseWeight + dielectricWeight + metalWeight + glassWeight + clearCoatWeight);
    diffuseWeight *= TotalWeightInv;
    dielectricWeight *= TotalWeightInv;
    metalWeight *= TotalWeightInv;
    glassWeight *= TotalWeightInv;
    clearCoatWeight *= TotalWeightInv;
    ...
    if (reflect)
    {
        ...
        specStr *= specStr;
        vec3 f0 = mix(vec3(specStr) * mix(vec3(1.0f), tint, mat.SpecularTint), mat.Color.xyz, mat.Metallic);
        vec3 f90 = vec3(1.0F);
...
void SampleBRDF(inout BRDFSampleData data, in Material mat, in Surface surface)
{
    ...
    float clearCoatCDF = glassCDF + clearCoatWeight;
    ...
    else if (r1 < clearCoatCDF)
    {
        vec3 halfVector = GgxSampling(mat.ClearCoatRoughness * mat.ClearCoatRoughness, r3, r4);
        halfVector = TangentToWorld(surface.Tangent, surface.Bitangent, surface.Normal, halfVector);
